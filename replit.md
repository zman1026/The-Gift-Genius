...
**Camera-Based Product Search (Planned):** The app will feature camera-based image search using SerpApi's Google Lens API to add products by photographing them in stores. Implementation plan:
- **Technology Stack:** HTML5 getUserMedia API for camera capture, SerpApi Google Lens API for visual product recognition, Replit Object Storage for temporary image storage
- **User Flow:** (1) User opens camera interface via Add Item dialog, (2) Camera permission requested, (3) Live camera preview with capture button, (4) Photo captured and uploaded to Object Storage, (5) Image sent to SerpApi Lens API, (6) Product results displayed inline with search results, (7) User selects product to add to wishlist
- **Technical Requirements:** HTTPS required for camera access, `playsinline` attribute for iOS compatibility, facingMode switching for front/rear cameras, base64 or blob upload to backend
- **API Integration:** SerpApi endpoint `/search?engine=google_lens` with image upload, existing SERPAPI_KEY environment variable, pricing ~$0.01-0.015 per search (affordable for family use)
- **Backend Routes:** New endpoint `POST /api/search/image` accepting multipart/form-data image uploads, validates file size (<10MB), uploads to private Object Storage folder, calls SerpApi Lens API with image URL, returns mapped product results matching existing SerpApi shopping format, automatic cleanup of temporary images after 1 hour
- **Storage Lifecycle:** Images uploaded to `.private/camera-search/{userId}/{timestamp}.jpg` in Object Storage, backend stores image metadata in memory cache with 1-hour TTL, scheduled cleanup job removes expired images, users can retry failed searches within cache window
- **Error Handling:** Rate limiting (10 searches per user per hour), graceful degradation if SerpApi quota exceeded, validation errors for unsupported image formats, camera permission denial fallback to file upload, timeout handling for slow API responses
- **Security:** Camera permissions handled by browser, temporary images stored in private Object Storage folder, automatic cleanup after processing
- **Mobile Optimization:** Responsive camera preview, touch-friendly capture button, camera switch button for front/rear toggle, error handling for denied permissions or unavailable cameras
