...
interface ImportAmazonWishlistDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  familyId: string;
  targetUserId?: string;
  targetManagedProfileId?: string;
  targetName?: string;
}

export function ImportAmazonWishlistDialog({
  open,
  onOpenChange,
  familyId,
  targetUserId,
  targetManagedProfileId,
  targetName,
}: ImportAmazonWishlistDialogProps) {
  ...
  const importMutation = useMutation({
    mutationFn: async (items: AmazonItem[]) => {
      const response = await apiRequest("POST", "/api/import/amazon-wishlist/import", {
        items,
        familyId,
        targetUserId,
        targetManagedProfileId,
    ...
    onSuccess: (data: { 
      ...
      // Show toast for partial success or issues
      if (data.skippedCount && data.skippedCount > 0) {
        toast({
          title: `Imported ${data.importedCount} items`,
          description: `${data.skippedCount} items skipped (already in wishlist).`,
          variant: "default",
        });
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>
            {targetName 
              ? `Import items from an Amazon wishlist to ${targetName}'s list`
              : "Import items from an Amazon wishlist to your list"
            }
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="flex flex-col items-center justify-center py-8 gap-4">
            ...
            <div className="text-center">
              ...
              <p className="text-muted-foreground">
                Successfully imported {importedCount} item{importedCount !== 1 ? 's' : ''} to your wishlist.
              </p>
              {skippedCount > 0 && (
                <p className="text-sm text-muted-foreground mt-1">
                  {skippedCount} item{skippedCount !== 1 ? 's' : ''} skipped (already in wishlist)
                </p>
              )}
            </div>
